...
// 시퀄라이즈는 자동으로 post 테이블에 userId 컬럼을 추가한다.
db.User.hasMany(db.Post);
db.Post.belongsTo(db.User);

// user & career 연관관계 (user : career = 1 : N)
// 시퀄라이즈는 자동으로 career 테이블에 userId 컬럼을 추가한다.
db.User.hasMany(db.Career);
db.Post.belongsTo(db.User);

// user & apply 연관관계 (user : apply = 1 : N)
// 시퀄라이즈는 자동으로 apply 테이블에 userId 컬럼을 추가한다.
db.User.hasMany(db.Apply);
db.Apply.belongsTo(db.User);

// apply & post 연관관계 (apply : post = 1 : 1)
// 시퀄라이즈는 자동으로 Apply 테이블에 postId컬럼을 추가한다.
